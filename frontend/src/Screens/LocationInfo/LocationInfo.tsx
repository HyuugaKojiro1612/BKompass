...
export const LocationInfo = ({route} : any) => {
  const { data } = route.params;
  console.log("----------------");
  console.log("params", data);
  const convertImages = (images: any[]) => {
    return images.map((url: any, index: number) => ({
      id: `${index + 1}`,
      source: `{ uri: "${url}" }`,
    }));
  };
  // Sử dụng hàm convertImages để chuyển đổi định dạng images
  const convertedData = {
    ...data,
  ...
  console.log("----------------");
  const [locationData, setLocationData] = React.useState<any>(convertedData);
  ...
  const renderScene = SceneMap({
    overview: () => <Overview basicInfo={locationData}/>,
    review: () => <Review voteAndComment={locationData.voteAndComment} user={user}/>,
    photos: () => <Photos images={basicInfo.images}/>,
    intro: () => <Introduction intro={locationData.intro} />, // Assuming intro is a string
  });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.body}>
        <View style={styles.BasicInfoContainer}>
          <View>
            <Text style={styles.BasicName}>{locationData.displayName}</Text>
            <Text style={styles.BasicLocation}>{locationData.location}</Text>
          </View>
        ...
        <ScrollView horizontal={true} showsHorizontalScrollIndicator={false} style={styles.wrapper}>
          {locationData && locationData.images && images.slice(0, 5).map((image: { id: React.Key | null | undefined; source: ImageSourcePropType; }, index: any) => (
            <View
              style={{
                borderRadius: 10,
